...
from picofan import com, things
from picofan.things import ThingKind

print(sys.path)

UART0 = com.JsonSerial(
    0,
    19200,
    tx=Pin(1),
    rx=Pin(2),
...
def handle_receive_command(uart: com.JsonSerial) -> None:
    micropython.schedule(parse_command, uart.read_json())
...
def parse_command(command: dict[str, Any]) -> None:
    match command["command"]:
        ...
        case CommandType.GET:
            details = tm.get(kind=command["kind"], address=command["addr"]).__dict__
            message = {
                "kind": command["kind"],
                "addr": command["addr"],
                "details": details,
        ...
        case CommandType.SET:
            setattr(
                tm.get(kind=command["kind"], address=command["addr"]),
                command["param"]["name"],
                command["param"]["value"],
            )
...
tm.register(
    ThingKind.FAN, 0, things.Fan(tach_pin=Pin(5, Pin.IN), pwm_pin=Pin(4, Pin.OUT))
)
tm.register(
    ThingKind.CURVE,
    0,
    things.FanCurve(
        therm=tm.get(ThingKind.THERMISTOR, 0), fans=[tm.get(ThingKind.FAN, 0)]
    ),
)
